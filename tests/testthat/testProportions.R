...
test_that("errors are generated when invalid arguments are used",{
  ...
  expect_error(phe_proportion(100,65),"numerators must be less than or equal to denominator
               for a proportion statistic")
  expect_error(phe_proportion(65,100,conf.level=20),"confidence level must be >= 90 and
               <= 100 (or >= 0.9 and <= 1)")
