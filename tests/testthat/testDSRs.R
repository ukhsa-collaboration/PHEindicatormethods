...
test_that("dsrs and CIs calculate correctly",{
  ...
  expect_equal(data.frame(select(calculate_dsr(test_multiarea_esp, count, pop),1:6)),
               data.frame(test_DSR_results[5:7,1:6]),
               ignore_attr = TRUE,info="test default")

  expect_equal(data.frame(select(calculate_dsr(test_multiarea_esp, count, pop, confidence = c(0.95,0.998)),1:8,10:11)),
               data.frame(test_DSR_results[5:7,]),
               ignore_attr = TRUE,info="test full output with 2 CIs")

  expect_equal(data.frame(calculate_dsr(test_DSR_1976, count, pop, stdpop = esp1976, type="standard")),
               data.frame(select(slice(test_DSR_results,8),2:6)),
               ignore_attr = TRUE,info="test with user specified stdpop by col name")

  expect_equal(data.frame(calculate_dsr(test_multiarea_esp, count, pop, stdpop = esp2013, type="standard")),
               data.frame(select(slice(test_DSR_results,5:7),1:6)),
               ignore_attr = TRUE,info="test standard")

  expect_equal(data.frame(calculate_dsr(test_multiarea_esp, count, pop, confidence = c(0.95,0.998), type="standard")),
               data.frame(select(slice(test_DSR_results,5:7),1:8)),
               ignore_attr = TRUE,info="test standard 2CIs")

  expect_equal(data.frame(calculate_dsr(test_multiarea_esp, count, pop, type="value")),
               data.frame(select(slice(test_DSR_results,5:7),1,4)),
               ignore_attr = TRUE,info="test value")

  expect_equal(data.frame(calculate_dsr(test_multiarea_esp, count, pop, confidence = c(0.95,0.998), type="value")),
               data.frame(select(slice(test_DSR_results,5:7),1,4)),
...
test_that("dsrs - errors are generated when invalid arguments are used",{
  ...
  expect_error(calculate_dsr(test_multiarea_esp, count),
  ...
  expect_error(calculate_dsr(test_multiarea_esp, count, pop, stdpop = esp2013, confidence = 0.74),
               "confidence level must be between 90 and 100 or between 0.9 and 1",info="error confidence < 0.9")

  expect_error(calculate_dsr(test_multiarea_esp, count, pop, stdpop = esp2013, confidence = 3),
               "confidence level must be between 90 and 100 or between 0.9 and 1",info="error confidence between 1 and 90")

  expect_error(calculate_dsr(test_multiarea_esp, count, pop, stdpop = esp2013, confidence = 1000),
               "confidence level must be between 90 and 100 or between 0.9 and 1",info="error confidence >100")

  expect_error(calculate_dsr(test_multiarea_esp, count, pop, stdpop = esp2013, type="combined"),
               "type must be one of value, lower, upper, standard or full",info="error invalid type")

  expect_error(calculate_dsr(filter(test_multiarea_esp,count < 100), count, pop, stdpop = esp2013),
               "data must contain the same number of rows for each group",info="error num rows per group")

  expect_error(calculate_dsr(test_DSR_1976, count, pop, stdpop = esp),
               "stdpop is not a field name from data",info="error stdpop field doesn't exist")

  expect_error(calculate_dsr(test_multiarea_esp, count, pop, confidence = c(90.95, 0.998, 1.00)),
               "a maximum of two confidence levels can be provided",info="error more than 2 CIs requested")

  expect_error(calculate_dsr(test_multiarea_esp, count, pop, confidence = c(0.95, 1.00)),
