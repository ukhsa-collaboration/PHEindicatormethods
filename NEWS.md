## PHEindicatormethods v2.0.WIP
* `phe_dsr` soft deprecated and replaced with `calculate_dsr`. The new calculate_dsr function allows the user to specify whether events are independent and performs and adjustment to the confidence interval calculation for non-independent events. Additionally, the new function requires the user to provide the standard populations in the input dataframe to negate the risk if incorrect joining by position and therefore the stdpoptype argument has been deprecated. The deprecated phe_dsr function will now issue a warning to inform users that they need to replace it with the new calculate_dsr function before September 2025 after which the phe_dsr function will be removed from the package. Previous calls to phe_dsr can be replaced with calculate_dsr without breaking changes if stdpoptype was set to "field" (the stdpoptype argument specification will just need to be removed). Where phe_dsr has been executed with stdpoptype = "vector" the user will need to append the standard populations to their input data frame before calling calculate_dsr.
