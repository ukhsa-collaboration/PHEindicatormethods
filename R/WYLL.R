# -------------------------------------------------------------------------------------------------
#' Calculate working years of life lost using phe_wyll
#'
...
phe_wyll <- function(data, x, n, leadj, stdpop = esp2013, stdpoptype = "vector",
                    type = "full", confidence = 0.95, multiplier = 100000) {

  # check required arguments present
  if (missing(data)|missing(x)|missing(n)|missing(leadj)) {
    stop("function phe_wyll requires at least 4 arguments: data, x, n, leadj")
  }
  ...
  if (length(confidence) == 2) {
    ...
    phe_wyll <- data %>%
      mutate(yll=({{leadj}}*{{x}}*(stdpop_calc/{{n}})),
             ...
             err_frac =((stdpop_calc/{{n}})^2)*({{x}})*(({{leadj}}))) %>%
